...
export default function ChapterMap({ progress, onSelectChapter }: { progress: any; onSelectChapter: (id: number) => void }): JSX.Element {
  const { progress: state, isChapterUnlocked } = progress;

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {chapters.map((chapter: any) => {
        const prevChapter = chapters.find((c: any) => c.id === chapter.id - 1);
        ...
        let status: 'playable' | 'locked' | 'coming-soon';
        if (playable) status = 'playable';
        else if (inRoadmap) status = 'locked';
        else status = 'coming-soon';

        const completedCount = Array.from({ length: chapter.missionCount }, (_: unknown, i: number) => i + 1).filter(
          (missionId: number) => state.completedMissions[`${chapter.id}-${missionId}`]
        ).length;
